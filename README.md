...
- [ ] 当我们点击一个链接，跳转到其它页面，然后回退的结果并不是我们原先跳的位置，那么如何记录原先跳的位置呢？——前端路由
- [ ] 添加个「[正在热映](https://web.archive.org/web/20151004012429/http://developers.douban.com/wiki/?title=movie_v2#in_theaters)」的tab，它是个商务接口，不知为啥，关于对数据请求是可以用的，而高级接口则不可以
...
- [ ] 有些电影条目的数据是没有的，如导演什么的……所以是不是要给一个默认值
